...
# Set up logging
def setup_logging(model_type="bert"):
    """Set up logging with model type-specific file name"""
    log_filename = f"{model_type}_icd9_training.log"
    logging.basicConfig(
        level=logging.INFO,
...
def save_model_info(model, section_names, binary_columns, numerical_columns, save_path, model_type="bert"):
    """
    ...
    Args:
        ...
        model_type: Type of BERT model used (bert or clinical_bert)
    """
    model_info = {
...
def parse_args():
    ...
    parser.add_argument("--model_type", type=str, default="bert", choices=["bert", "clinical_bert"], 
                       help="Type of BERT model to use (bert or clinical_bert)")
...
def main():
    ...
    global logger
    logger = setup_logging(model_type=args.model_type)
    ...
    if args.model_type == "clinical_bert":
        output_dir = output_dir.parent / "clinical_bert_icd9"
    output_dir.mkdir(parents=True, exist_ok=True)
    
    # Generate timestamp for unique run
    timestamp = datetime.datetime.now().strftime("%Y%m%d_%H%M%S")
    run_dir = output_dir / f"{args.model_type}_run_{timestamp}"
    ...
    # Determine BERT model to use
    if args.model_type == "clinical_bert":
        bert_model_name = "emilyalsentzer/Bio_ClinicalBERT"
        logger.info(f"Using Clinical BERT model: {bert_model_name}")
    else:
        bert_model_name = args.model_name
        logger.info(f"Using standard BERT model: {bert_model_name}")
    ...
    # Save model info
    save_model_info(model, section_names, binary_columns, numerical_columns, run_dir, model_type=args.model_type)
    ...
    torch.save(model.state_dict(), output_dir / f"{args.model_type}_best_model.pt")
